...
function isSessionExpiring(session) {
  ...
  return (Number(session.expiresAt) * 1000) - Date.now() < 120000;
}

async function refreshSession(force = false) {
  const session = loadSession();
  if (!session?.refreshToken) return session;
  if (!force && !isSessionExpiring(session)) return session;
...
async function apiRequest(url, options = {}) {
  ...
  if (response.status === 401 && loadSession()?.refreshToken) {
        ...
        <article class="panel">
          ...
          <form id="scale-form" class="form">
            <label>Funcionário
              ...
              <small class="field-note">Lista reduzida para nome curto e matrícula.</small>
            </label>
            <label>Turno
            ...
            <label>Equipe
              ...
              <small class="field-note">As sugestões usam apenas o primeiro e segundo nome.</small>
            </label>
            ${teamDataList()}
